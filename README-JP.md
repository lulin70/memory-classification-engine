...
MCE には MCP サーバーが組み込まれています。Claude Code / Cursor など MCP 対応ツールで MCE を使う最速の方法です。
...
詳細な設定手順は [BETA_TESTING_GUIDE_EN.md](./BETA_TESTING_GUIDE_EN.md) を参照してください。
...
| 指標 | 結果 |
|------|------|
| メッセージ処理（Layer 1/2） | ~10ms |
| メッセージ処理（Layer 3） | <500ms |
| 検索レイテンシ | ~15ms |
| 同時スループット | 626 msg/s |
| 記憶圧縮率 | 87-90% ノイズ削減 |
| メモリ使用量 | <100MB（基本モード） |
...
- Beta テストガイド: [BETA_TESTING_GUIDE_EN.md](./BETA_TESTING_GUIDE_EN.md)
- Issues / Discussions
