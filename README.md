...
- **Sentiment Marker** (`sentiment_marker`): User's explicit emotional倾向 towards a topic
...
| Learning Ability | Basic | None | None | None | Supports extracting procedural memory from episodic memory<br><small>Note: Repeated patterns automatically promote to rules,实现 memory evolution</small> |
...
Please refer to the [CONTRIBUTING.md](CONTRIBUTING.md) file for detailed contribution流程.
