# CarryMem 产品共识 × 架构决策 × 四方评审 (v5.0)
**日期**: 2026-04-21 (v5.0 更新)
**决策**: **NotebookLM 启发三个方向落地 — 不做 UI，做数据 API**
**输入**: Gemini × NotebookLM 打通分析 + 创始人初心 + v4.0共识
**参与方**: PM / ARCH / QA / DEV / UI

**v5.0 核心变更**:
- 新增 §11: NotebookLM 启发与 CarryMem 响应
- 三个方向落地: 记忆画像(v0.8✅) + 项目空间(v0.9📋) + 主动声明(v0.8✅)
- MCP 工具: 3+3+3 → **3+3+3+2** (新增 Profile 组: declare_preference, get_memory_profile)
- 关键原则: CarryMem 不做 UI，做数据 API
...
## 11. NotebookLM 启发与 CarryMem 响应 (v5.0, 2026-04-21)

### 11.1 背景

Google 宣布 Gemini 与 NotebookLM 深度打通，推出 Notebooks 功能：
- **跨对话永久记忆**：同一 Notebook 下的所有对话共享记忆库
- **自定义指令绑定记忆**：用户在项目空间填写指令，AI 持续遵循
- **聊天记录转知识资产**：对话内容一键转思维导图、播客、PPT

### 11.2 核心判断

NotebookLM 的"记忆可视化"和"项目空间"验证了用户需求，但 CarryMem 的响应方式不同——**不做 UI，做数据 API**。

| 维度 | NotebookLM | CarryMem |
|------|-----------|----------|
| 产品形态 | 终端用户应用 | 开发者中间件 |
| 记忆可视化 | 生成式输出（播客/PPT/思维导图） | 结构化聚合（get_memory_profile） |
| 项目空间 | 完整 Notebook UI | namespace 字段（轻量隔离） |
| 主动声明 | 自定义指令 | declare() API |
| 数据所有权 | 锁在 Google | 属于用户（SQLite 本地文件） |

### 11.3 三个方向

#### 方向1: 记忆画像 (get_memory_profile) — v0.8 ✅

**理念**：让记忆对用户可见、可审计、可掌控。数据在 CarryMem，展示在 Agent。
...
- 信息密度更高，更符合"只记值得记的"理念
- 任何 Agent UI 都可以消费这个 JSON

#### 方向2: 项目空间 (namespace) — v0.9 📋

**理念**：轻量 namespace 字段，不是完整的"项目"概念。

```python
cm = CarryMem(storage="sqlite", namespace="project-alpha")
# → 存入 memories 表，namespace="project-alpha"
# → 查询时只返回当前 namespace 的记忆
# → 跨项目查询：recall_all(namespace=["project-alpha", "global"])
```

**检索优先级 (v0.9+)**：
```
Layer 1a: 当前 namespace 的记忆 (project-specific)
Layer 1b: 全局 namespace 的记忆 (user-level preferences/corrections)
Layer 2:  知识库 (Obsidian)
Layer 3:  外部 LLM (Agent 决定)
```

**时机**：存储层稳定后再推进（v0.9）。

#### 方向3: 主动声明 (declare) — v0.8 ✅

**理念**：主动声明也经过分类引擎，但 confidence=1.0, source_layer="declaration"。

```python
result = cm.declare("I prefer dark mode in my editor")
# → 分类引擎仍然运行，产出 MemoryEntry
# → 但 source_layer="declaration", confidence=1.0
# → suggested_action="store" (用户主动说的，一定存)
```
...
**两条路并行**：主动声明 + 被动分类 = 完整的记忆收集体验。

### 11.4 五方评审

| 角色 | 投票 | 核心判断 | 顾虑 | 解决方案 |
|------|------|---------|------|---------|
| 👔 PM | ✅ | 记忆画像性价比最高，主动声明补全体验 | 记忆面板是中间件功能还是应用功能？ | 不做 UI，做数据 API |
| 🏗️ ARCH | ✅ | declare() 架构最干净，profile() 零风险 | namespace 的 Schema 影响和迁移 | v0.9 再做，先加 namespace 列 |
| 🧪 QA | ✅ | 测试场景清晰 | namespace 隔离验证 | v0.9 补充 |
| 💻 DEV | ✅ | declare ~40行, profile ~80行, 风险极低 | — | 先3后1再2 |
| 🎨 UI | ✅ | 数据可展示比自己做UI更重要 | — | highlights 只返回 top N |
