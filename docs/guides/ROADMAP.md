...
### v0.4.0 — Semantic Recall (🆕)
- **SemanticExpander** — Zero-dependency semantic query expansion
  - Synonym graph (同义词图谱): 470+ terms across CN/EN/JP
  - Spell correction (编辑距离): Levenshtein distance ≤ 2
  - Cross-language mapping: CN ↔ EN ↔ JP translation pairs
- **ResultMerger** — Multi-source result fusion and ranking
  - Deduplication by storage_key
  - Relevance scoring with source-weighted ranking
  - Configurable min_relevance threshold
- **SQLiteAdapter Integration** — Transparent semantic recall in recall()
  - Phase 1: FTS5 exact match (existing, unchanged)
  - Phase 2: Semantic expansion (auto-triggered when results insufficient)
  - Phase 3: Result fusion (merged + ranked output)
- **199 tests passing** (133 original + 66 new semantic tests)
- **Performance**: <100ms recall on 500 memories
...
- 199 tests passing (EN×7 + ZH×7 + JA×7 + noise×3 + integration + export/import + edge + semantic×66)
