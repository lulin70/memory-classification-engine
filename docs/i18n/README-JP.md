...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-199%20passing-green" alt="Tests">
  <img src="https://img.shadge.io/badge/semantic%20recall-%E2%9C%85-brightgreen" alt="Semantic Recall">
...
```python
# 中国語で保存、英語で検索 — 言語を超えて動作！
cm.classify_and_remember("我偏好使用PostgreSQL")

# 以下のすべてのクエリがメモリを見つけます：
memories = cm.recall_memories("PostgreSQL")     # ✅ 完全一致
memories = cm.recall_memories("数据库")          # ✅ シノニム拡張
memories = cm.recall_memories("Postgres")        # ✅ スペル訂正
memories = cm.recall_memories("データベース")    # ✅ クロスラングauge（日本語）
# → すべてPostgreSQLの好みメモリを返します！
```

**機能：**
- 🔄 **シノニム拡張**: "数据库" → PostgreSQL, MySQL, Redis...
- ✨ **スペル訂正**: "Postgres" → "PostgreSQL"
- 🌐 **クロスラングauge**: 中 ↔ 英 ↔ 日
- ⚡ **ゼロ依存**: PyTorch不要、外部モデル不要
- 🔧 **設定可能**: ランタイムで有効/無効化
